...
make_helper(movsreg) {
  ...
  SegDesc tmp;
  int x=swaddr_read(sreg_index(op_dest->sreg)+cpu.GDTR.base, 4, R_CS);
  memcpy((void*)&tmp, (void*)&x, 4);
  x=swaddr_read(sreg_index(op_dest->sreg)+cpu.GDTR.base+4, 4, R_CS);
  ...
  printf("sreg: %x %x %x\n",op_dest->sreg,  SD.Merge.limit, SD.Merge.base);
  return len+1;
}
#endif
